...
void el2csr(char *el, int &m, int &nnz, IndexT *&row_offsets, IndexT *&column_indices, WeightT *&weight, bool symmetrize, bool transpose, bool sorted, bool remove_selfloops, bool remove_redundents) {
	...
	// Always count edges to handle web-Google.txt format correctly
	cfile.close();
	cfile.open(el);
	// Skip comments again
	while (true) {
	...
	nnz = 0;
	m = 0;
	std::vector<std::pair<IndexT, IndexT>> edges;
	IndexT src, dst;
	do {
		if (sscanf(str.c_str(), "%d %d", &src, &dst) == 2) {
	...
	m++;
	// Now create vertices and fill data
	vector<vector<WeightedEdge>> vertices;
	vector<WeightedEdge> neighbors;
	for (int i = 0; i < m; i++)
		vertices.push_back(neighbors);
	...
	for (auto &edge : edges) {
		src = edge.first;
		dst = edge.second;
		// Don't decrement src and dst since web-Google.el uses 0-based indexing
		WeightedEdge e1, e2;
...
void read_graph(int argc, char *argv[], int &m, int &n, int &nnz, IndexT *&row_offsets, IndexT *&column_indices, int *&degree, WeightT *&weight, bool is_symmetrize=false, bool is_transpose=false, bool sorted=true, bool remove_selfloops=true, bool remove_redundents=true) {
	...
	if (dot) {
		...
		printf("Comparing with .mtx: %d\n", strcasecmp(dot, ".mtx"));
		printf("Comparing with .graph: %d\n", strcasecmp(dot, ".graph"));
		printf("Comparing with .gr: %d\n", strcasecmp(dot, ".gr"));
		printf("Comparing with .el: %d\n", strcasecmp(dot, ".el"));
		printf("Comparing with .txt: %d\n", strcasecmp(dot, ".txt"));
		if (strcasecmp(dot, ".mtx") == 0) {
			printf("Recognized .mtx format\n");
		...
		} else if (strcasecmp(dot, ".graph") == 0) {
			printf("Recognized .graph format\n");
		...
		} else if (strcasecmp(dot, ".gr") == 0) {
			printf("Recognized .gr format\n");
		...
		} else if (strcasecmp(dot, ".el") == 0 || strcasecmp(dot, ".txt") == 0) {
			printf("Recognized .el/.txt format\n");
			el2csr(argv[1], m, nnz, row_offsets, column_indices, weight, is_symmetrize, is_transpose, sorted, remove_selfloops, remove_redundents);
		} else {
			printf("Unrecognizable input file format: %s\n", dot);
			exit(0);
		}
	} else {
